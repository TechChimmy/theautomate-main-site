...
export async function POST(req: NextRequest) {
  try {
    const { email, productUuid: providedProductUuid, targetBundleTitle, targetBundlePrice } = await req.json();

    if (!email || !targetBundleTitle || targetBundlePrice === undefined) {
      return NextResponse.json(
        { error: "Email, targetBundleTitle, and targetBundlePrice are required." },
        { status: 400 }
      );
    }
    ...
    if (!profile) return NextResponse.json({ eligible: false, price: fullPrice });
    ...
    if (!phase2User) return NextResponse.json({ eligible: false, price: fullPrice });
    ...
    if (providedProductUuid) {
      enrollmentQuery = enrollmentQuery.or(`course_id.eq.${providedProductUuid},product_id.eq.${providedProductUuid}`);
    }

    const { data: enrollments, error: enrollmentsError } = await enrollmentQuery;

    if (!enrollments || enrollments.length === 0) return NextResponse.json({ eligible: false, price: fullPrice });
    ...
    if (!enrollment) return NextResponse.json({ eligible: false, price: fullPrice, message: "No eligible enrollment found to upgrade from." });

    const resolvedCourseId = enrollment.course_id;
    const resolvedProductUuid = enrollment.product_id || enrollment.course_id;
    const resolvedBatchType = enrollment.batch_type || "weekend";
    ...
    if (targetLevel <= currentLevel) {
      return NextResponse.json({ eligible: false, price: fullPrice, message: "Cannot downgrade or buy same tier." });
    }
    ...
    if (diffHours > 120) {
      return NextResponse.json({ eligible: false, price: fullPrice, message: "Outside upgrade window." });
    }
    ...
    if (orders && orders.length > 0) {
      ...
      const orderWithPhone = orders.find(o => o.phone_number && o.phone_number.trim() !== "");
      if (orderWithPhone) {
